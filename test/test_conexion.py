...
Versión: 2.2 - Con métodos de auditoría
"""
...
def prueba_1_conexion():
    ...
    if db.conectar():
        ...
        db.desconectar()
        registrar_prueba("Conexión Básica", True, "Conexión exitosa")
        return True
    else:
        print("\n❌ FALLÓ: No se pudo conectar")
        registrar_prueba("Conexión Básica", False, "No se pudo conectar")
        return False
...
def prueba_2_listar_tablas():
    ...
    if tablas:
        ...
        db.desconectar()
        registrar_prueba("Listar Tablas", True, f"{len(tablas)} tablas encontradas")
        return True
    else:
        print("\n❌ FALLÓ: No se encontraron tablas")
        db.desconectar()
        registrar_prueba("Listar Tablas", False, "No se encontraron tablas")
        return False
...
def prueba_3_insert():
    ...
    if nuevo_id:
        ...
        db.desconectar()
        registrar_prueba("INSERT", True, f"Cliente insertado ID {nuevo_id}")
        return True
    else:
        print("\n❌ FALLÓ: Error en INSERT")
        db.desconectar()
        registrar_prueba("INSERT", False, "Error en INSERT")
        return False
...
def prueba_4_select():
    ...
    if not test_id:
        ...
        db.desconectar()
        registrar_prueba("SELECT", False, "No se pudo insertar dato de prueba")
        return False
    ...
    if resultados and len(resultados) > 0:
        ...
        db.desconectar()
        registrar_prueba("SELECT", True, f"SELECT exitoso para ID {test_id}")
        return True
    else:
        print("\n❌ FALLÓ: No se encontró el registro")
        db.desconectar()
        registrar_prueba("SELECT", False, "No se encontró el registro")
        return False
