...
def setup_datos_prueba():
    """Crea datos necesarios para las pruebas"""
    ...
    exito, msg = inv_ctrl.agregar_disfraz(
...
def prueba_4_venta_con_descuento():
    ...
    if exito:
        ...
        if venta and venta.descuento_porcentaje == 20:
            ...
            print(f"   Total: ${venta.total_con_descuento():.2f}\n")
            return True
...
def prueba_10_cancelar_venta_admin():
    ...
    if exito:
        # Forzar nueva consulta con nueva conexión
        import time
        time.sleep(0.1)  # Pequeña pausa para asegurar commit
        
        # Crear nuevo controlador para forzar nueva conexión
        ...
        if stock_despues > stock_antes:
            ...
            # Verificar en BD directamente como última opción
            from config.database import ConexionDB
            db = ConexionDB()
            db.conectar()
            query_check = "SELECT Stock, Disponible FROM INVENTARIO WHERE Codigo_Barras = %s"
            resultado = db.ejecutar_query(query_check, (codigo_disfraz_test,))
            if resultado:
                stock_real, disp_real = resultado[0]
                print(f"   Verificación directa BD: Stock={stock_real}, Disponible={disp_real}")
                if disp_real > stock_antes:
                    print(f"✅ PASÓ: Stock devuelto verificado en BD\n")
                    return True
...
def limpiar_datos_prueba():
    ...
    db.ejecutar_update("DELETE FROM DETALLE_VENTAS WHERE Id_Venta IN (SELECT Id_Venta FROM VENTAS WHERE Id_cliente = %s)", (id_cliente_test,))
...
def ejecutar_todas():
    ...
    if exitosas == total:
        ...
        print("✅ Control transaccional: OK")
