...
Versión: 3.3 - ✅ Corregido: siempre descuenta stock (modo personalizado y automático).
            ✅ Mantiene retrocompatibilidad con llamadas existentes.
            ✅ Soporta total y depósito personalizados desde formulario.
"""
...
class RentaController:
    ...
    Características:
    - Registrar rentas con detalles, depósito opcional y descuentos personalizados.
    - Siempre descuenta Disponible del inventario (¡evita dobles rentas!).
    - Soporta modo automático (total = suma de subtotales, depósito = precio_venta × cantidad).
    - Soporta modo personalizado (total y depósito definidos por el usuario).
    ...
    def _validar_datos_renta_basicos(
        ...
        try:
            ...
            logger.error(f"Error de base de datos validando usuario: {e}")
    ...
    # ============================================================
    # REGISTRAR RENTA (usado por test_rentas.py y formulario v4.1+)
    # ============================================================
    ...
    def registrar_renta(
        ...
        detalles: List[Dict[str, Any]],
        dias_renta: int,
        total_personalizado: Optional[Decimal] = None,
        deposito_personalizado: Optional[Decimal] = None
    ) -> Tuple[bool, str, Optional[int]]:
        """
        Registra una renta completa con detalles.

        Parámetros nuevos (opcionales):
            total_personalizado: Total final definido por el usuario (incluye descuentos).
            deposito_personalizado: Depósito definido por el usuario.

        Comportamiento:
            - Si se proporcionan total_personalizado y deposito_personalizado:
                → Usa esos valores directamente (valida que no sean negativos).
                → Verifica disponibilidad antes de registrar.
            - Si NO se proporcionan:
                → Usa lógica original (total = suma de subtotales, depósito = precio_venta × cantidad).
                → Verifica disponibilidad y precios como antes.
            - ✅ SIEMPRE descuenta Disponible del inventario.
        """
        ...
        try:
            ...
            modo_personalizado = (total_personalizado is not None and deposito_personalizado is not None)

            if modo_personalizado:
                total = Decimal(str(total_personalizado))
                deposito = Decimal(str(deposito_personalizado))

                if total < 0:
                    logger.warning(f"Total personalizado negativo: {total}")
                    return False, "El total no puede ser negativo", None
                if deposito < 0:
                    logger.warning(f"Depósito personalizado negativo: {deposito}")
                    return False, "El depósito no puede ser negativo", None

            # === Verificar disponibilidad y calcular totales si es automático ===
            if not modo_personalizado:
                for item in detalles:
                    ...
                    if precio_renta < 0 or precio_venta < 0:
                        logger.warning(f"Precios negativos para {codigo}")
                        return False, f"Precios no pueden ser negativos para {codigo}", None
            ...
            nuevo_id = self.db.ejecutar_insert(
                q_renta,
                (id_cliente, id_usuario, ahora, fecha_devolucion, dias_renta, float(total), float(deposito))
            )
            ...
            # === Insertar detalles y descontar stock (SIEMPRE) ===
            for item in detalles:
                codigo = item['codigo_barras']
                cantidad = int(item['cantidad'])

                # Verificar disponibilidad (si no se hizo antes)
                if modo_personalizado:
                    hay, msg_disp, _ = self.inv_controller.verificar_disponibilidad(codigo, cantidad)
                    if not hay:
                        logger.info(f"Stock insuficiente en modo personalizado para {codigo}: {msg_disp}")
                        return False, f"Sin stock suficiente para {codigo}: {msg_disp}", None

                disfraz = self.inv_controller.buscar_por_codigo(codigo)
                if not disfraz:
                    logger.warning(f"Disfraz no encontrado al insertar detalle: {codigo}")
                    continue
                ...
                self.db.ejecutar_insert(
                    q_det,
                    (nuevo_id, codigo, cantidad, float(precio_renta), float(subtotal))
                )
            ...
            return True, "Renta registrada correctamente", nuevo_id
    ...
    def devolver_renta(
        ...
        try:
            ...
            ahora = datetime.now()
            q_det = "SELECT Codigo_Barras, Cantidad FROM DETALLE_RENTAS WHERE Id_Renta = %s"
            ...
            q_upd = """
                UPDATE RENTAS
                SET Fecha_Devuelto = %s, Penalizacion = %s, Estado = 'Devuelto'
                WHERE Id_Renta = %s
            """
            ...
            logger.info(
                f"Renta ID {id_renta} devuelta. Días retraso: {dias_retraso}, "
                f"Penalización: {penalizacion}, Depósito devuelto: {renta.deposito}"
            )
    ...
    def marcar_rentas_vencidas(self) -> int:
        try:
            self.db.conectar()
            ahora = datetime.now()
            q = "UPDATE RENTAS SET Estado = 'Vencida' WHERE Estado = 'Activa' AND Fecha_Devolucion < %s"
    ...
    def listar_rentas_activas(self) -> List[Renta]:
        try:
            self.db.conectar()
            q = "SELECT * FROM RENTAS WHERE Estado IN ('Activa', 'Vencida') ORDER BY Fecha_Renta DESC"
            r = self.db.ejecutar_query(q)
    ...
    def contar_rentas_activas(self) -> int:
        try:
            self.db.conectar()
            q = "SELECT COUNT(*) FROM RENTAS WHERE Estado IN ('Activa', 'Vencida')"
            r = self.db.ejecutar_query(q)
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("PRUEBA - RentaController v3.3")
    print("✅ Corrección: stock coherente en modo personalizado")
    print("="*60 + "\n")
    
    rc = RentaController()
    print("1️⃣ Penalización por día actual: ${:.2f}".format(rc.penalizacion_dia))
    print("2️⃣ Contando rentas activas... Total: {}".format(rc.contar_rentas_activas()))
    print("3️⃣ Listando rentas activas... Encontradas: {}".format(len(rc.listar_rentas_activas())))
    print("="*60 + "\n")