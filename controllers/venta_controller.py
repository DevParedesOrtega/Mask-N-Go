...
Versión: 2.6 - Corregido: obtener_resumen_ventas con campos correctos y alias consistente
"""
...
class VentaController:
    ...
    def generar_folio(self) -> str:
        """
        Genera un folio único para la venta con formato YYYYMMDD-NNNNN
        Ejemplo: 20251211-0001

        Returns:
            str: Folio único generado (máximo 15 caracteres)
        """
        try:
            self.db.conectar()

            ahora = datetime.now()
            # Formato de fecha: AñoMesDía
            fecha_str = ahora.strftime("%Y%m%d")

            # Consulta para obtener el último folio del día
            query_ultimo = """
                SELECT MAX(Folio) as ultimo_folio 
                FROM VENTAS 
                WHERE DATE(Fecha_venta) = %s AND Folio LIKE %s
            """
            resultado_ultimo = self.db.ejecutar_query(query_ultimo, (ahora.date(), f"{fecha_str}-%"))

            # Extraer el número secuencial
            numero_secuencial = 1
            if resultado_ultimo and resultado_ultimo[0][0]:
                ultimo_folio = resultado_ultimo[0][0]
                # Extraer el número después del guión
                try:
                    numero_secuencial = int(ultimo_folio.split('-')[1]) + 1
                except (IndexError, ValueError):
                    numero_secuencial = 1

            # Formatear el número secuencial con 5 dígitos (puedes ajustar esto)
            numero_formateado = f"{numero_secuencial:05d}"

            folio = f"{fecha_str}-{numero_formateado}"
